    ...
    try {
        ...
        if (orphaned.length > 0) {
            ...
            for (const order of orphaned) {
                // Determine new branch handling (shift the last element to the 10th index if it was remarks)
                let newRow = [...order.row];
                // Try appending it to row 7 + (since there's empty rows above 1000)
